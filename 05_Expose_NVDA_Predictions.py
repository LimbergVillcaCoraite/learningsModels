...
# Tabla historica de pronosticos (append para auditoria)
(
    forecast_df.select(
        "symbol",
        "forecast_date",
    ...
    .format("delta")
    .mode("append")
    .saveAsTable(serving_history_table)
)
...
metrics_df.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable(serving_metrics_table)
